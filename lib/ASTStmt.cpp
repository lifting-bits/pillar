...
#define MATCH_CK(ck) case vast::hl::CastKind::ck: return clang::CK_ ## ck;
  switch (vck) {
    MATCH_CK(NoOp)
...
clang::Expr *AST::LiftAShrOp(clang::DeclContext *dc, mlir::Operation &op_) {
  vast::hl::BinShrOp op = mlir::dyn_cast<vast::hl::AShrOp>(op_);
  clang::Expr *lhs_expr = LiftValue(dc, op.getLhs());
  clang::Expr *rhs_expr = LiftValue(dc, op.getRhs());
  assert(lhs_expr != nullptr);
  assert(rhs_expr != nullptr);
...
clang::Expr *AST::LiftLShrOp(clang::DeclContext *dc, mlir::Operation &op_) {
  vast::hl::BinShrOp op = mlir::dyn_cast<vast::hl::LShrOp>(op_);
  clang::Expr *lhs_expr = LiftValue(dc, op.getLhs());
  clang::Expr *rhs_expr = LiftValue(dc, op.getRhs());
  assert(lhs_expr != nullptr);
  assert(rhs_expr != nullptr);
...
clang::Expr *AST::LiftBinShlOp(clang::DeclContext *dc, mlir::Operation &op_) {
...
clang::Expr *AST::LiftConstantOp(clang::DeclContext *dc,
  ...
  return vast::TypeSwitch<mlir::TypedAttr, clang::Expr *>(op.getValue())
      .Case<vast::hl::BooleanAttr>(
          [=] (vast::hl::BooleanAttr v) -> clang::Expr * {
            v.dump();
            assert(false);
            return nullptr;
          })
      .Case<vast::hl::IntegerAttr>(
          [=] (vast::hl::IntegerAttr v) -> clang::Expr * {
      ...
      .Case<vast::hl::FloatAttr>(
          [=] (vast::hl::FloatAttr v) -> clang::Expr * {
            return clang::FloatingLiteral::Create(
                ctx, v.getValue(), /* IsExact= */true, LiftType(v.getType()),
                kEmptyLoc);
          })
      .Case<mlir::StringAttr>(
          [=] (vast::hl::StringAttr v) -> clang::Expr * {
            v.dump();
            assert(false);
            return nullptr;
          })
      .Default(
          [=] (mlir::TypedAttr v) -> clang::Expr * {
            v.dump();
            assert(false);
            return nullptr;
          });
...
clang::Expr *AST::LiftCStyleCastOp(clang::DeclContext *dc,
  ...
  clang::ExprValueKind evk = op_type.isa<vast::hl::LValueType>() ?
...
clang::Stmt *AST::LiftOpImpl(clang::DeclContext *dc, mlir::Operation &op) {
  switch (KindOf(op)) {
    case HlOpKind::kUnknown:
      assert(false);
      return nullptr;
    case HlOpKind::kBinShlOp: return LiftBinShlOp(dc, op);
    case HlOpKind::kBinShrOp: return LiftBinShrOp(dc, op);
    case HlOpKind::kBinAndOp: return LiftBinAndOp(dc, op);
    case HlOpKind::kBinOrOp: return LiftBinOrOp(dc, op);
    case HlOpKind::kBinXorOp: return LiftBinXorOp(dc, op);
    case HlOpKind::kAddIOp: return LiftAddIOp(dc, op);
    case HlOpKind::kSubIOp: return LiftSubIOp(dc, op);
    case HlOpKind::kMulIOp: return LiftMulIOp(dc, op);
    case HlOpKind::kDivUOp: return LiftDivUOp(dc, op);
    case HlOpKind::kDivSOp: return LiftDivSOp(dc, op);
    case HlOpKind::kRemUOp: return LiftRemUOp(dc, op);
    case HlOpKind::kRemSOp: return LiftRemSOp(dc, op);
    case HlOpKind::kNotOp: return LiftNotOp(dc, op);
    case HlOpKind::kLNotOp: return LiftLNotOp(dc, op);
    case HlOpKind::kDeclRefOp: return LiftDeclRefOp(dc, op);
    case HlOpKind::kValueYieldOp: return LiftValueYieldOp(dc, op);
    case HlOpKind::kExprOp: return LiftExprOp(dc, op);
    case HlOpKind::kConstantOp: return LiftConstantOp(dc, op);
    case HlOpKind::kImplicitCastOp: return LiftImplicitCastOp(dc, op);
    case HlOpKind::kCStyleCastOp: return LiftCStyleCastOp(dc, op);
    case HlOpKind::kCondYieldOp: return LiftCondYieldOp(dc, op);
    case HlOpKind::kDoOp: return LiftDoOp(dc, op);
    case HlOpKind::kReturnOp: return LiftReturnOp(dc, op);
    default:
      op.dump();
      return nullptr;
  }
